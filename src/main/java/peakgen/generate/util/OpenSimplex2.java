...
/*
 * Source: https://github.com/KdotJPG/OpenSimplex2 Licence: Creative Commons Zero v1.0 Universal:
 * https://github.com/KdotJPG/OpenSimplex2/blob/master/LICENSE
 */
...
public class OpenSimplex2 {
  ...
  /**
   * 2D Simplex noise, with Y pointing down the main diagonal. Might be better for a 2D sandbox
   * style game, where Y is vertical. Probably slightly less optimal for heightmaps or continent
   * maps, unless your map is centered around an equator. It's a subtle difference, but the option
   * is here to make it an easy choice.
   */
  ...
  /**
   * 3D OpenSimplex2 noise, with better visual isotropy in (X, Y). Recommended for 3D terrain and
   * time-varied animations. The Z coordinate should always be the "different" coordinate in
   * whatever your use case is. If Y is vertical in world coordinates, call noise3_ImproveXZ(x, z,
   * Y) or use noise3_XZBeforeY. If Z is vertical in world coordinates, call noise3_ImproveXZ(x, y,
   * Z). For a time varied animation, call noise3_ImproveXY(x, y, T).
   */
  ...
  private static float noise3_UnrotatedBase(long seed, double xr, double yr, double zr) {
    ...
    int xNSign = (int) (-1.0f - xri) | 1,
        yNSign = (int) (-1.0f - yri) | 1,
        zNSign = (int) (-1.0f - zri) | 1;

    // Compute absolute values, using the above as a shortcut. This was faster in my tests for some
    // reason.
  ...
  /**
   * 3D OpenSimplex2 noise, with better visual isotropy in (X, Z). Recommended for 3D terrain and
   * time-varied animations. The Y coordinate should always be the "different" coordinate in
   * whatever your use case is. If Y is vertical in world coordinates, call noise3_ImproveXZ(x, Y,
   * z). If Z is vertical in world coordinates, call noise3_ImproveXZ(x, Z, y) or use
   * noise3_ImproveXY. For a time varied animation, call noise3_ImproveXZ(x, T, y) or use
   * noise3_ImproveXY.
   */
  ...
  /**
   * 3D OpenSimplex2 noise, fallback rotation option Use noise3_ImproveXY or noise3_ImproveXZ
   * instead, wherever appropriate. They have less diagonal bias. This function's best use is as a
   * fallback.
   */
  ...
  /**
   * 4D OpenSimplex2 noise, with XYZ oriented like noise3_ImproveXY and W for an extra degree of
  ...
  private static float noise4_UnskewedBase(long seed, double xs, double ys, double zs, double ws) {
    ...
    // Determine which lattice we can be confident has a contributing point its corresponding cell's
    // base simplex.
    // We only look at the spaces between the diagonal planes. This proved effective in all of my
    // tests.
    ...
    for (int i = 0; ; i++) {
      ...
      // Next point is the closest vertex on the 4-simplex whose base vertex is the aforementioned
      // vertex.
      double score0 =
          1.0 + ssi * (-1.0 / UNSKEW_4D); // Seems slightly faster than 1.0-xsi-ysi-zsi-wsi
  ...
  /**
   * 4D OpenSimplex2 noise, with XYZ oriented like noise3_ImproveXZ and W for an extra degree of
   * freedom. W repeats eventually. Recommended for time-varied animations which texture a 3D object
  ...
  /**
   * 4D OpenSimplex2 noise, with XYZ oriented like noise3_Fallback and W for an extra degree of
   * freedom. W repeats eventually. Recommended for time-varied animations which texture a 3D object
  ...
  /**
   * 4D OpenSimplex2 noise, with XY and ZW forming orthogonal triangular-based planes. Recommended
